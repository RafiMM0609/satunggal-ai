...
_INTENT_RULES: list[tuple[str, "frozenset[str] | None"]] = [
    ...
    (
        '20. CONTEXT-AWARE FOLLOW-UP ROUTING — Read this rule before classifying ANY message when\n'
        '    "Riwayat percakapan terakhir" is present.\n'
        '\n'
        '    STEP 1 – Detect the previous specialist agent from [Asisten] signals:\n'
        '    ┌─────────────────────────────┬──────────────────────────────────────────────────────┐\n'
        '    │ Previous agent              │ Signal phrases in the last [Asisten] message         │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ web_automation              │ "📍 URL Aktif", "klik tombol", "navigate", "halaman  │\n'
        '    │                             │  berhasil dibuka", "screenshot", "login berhasil",   │\n'
        '    │                             │  "form diisi", "Data Registrasi yang Digunakan"      │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ research                    │ "Berdasarkan hasil riset", "Sumber:", "[1]", "[2]",  │\n'
        '    │                             │  "temuan riset", "menurut sumber", "tautan referensi"│\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ code_inspection             │ "Bug ditemukan", "Root cause", "rekomendasi perbaikan"│\n'
        '    │                             │  "analisis kode", "issue #", "stack trace"           │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ code_development /          │ "Kode berhasil diperbaiki", "commit", "branch",       │\n'
        '    │ code_fix                    │  "pull request", "docker sandbox", "repo di-clone"   │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ code_understanding          │ "Tech stack:", "Endpoint:", "Data model:", "Alur:",   │\n'
        '    │ (developer_qna)             │  "Dependency:", "Arsitektur", "API yang tersedia"     │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ wbs_agent / mandays_agent   │ "WBS", "Work Breakdown Structure", "mandays",         │\n'
        '    │                             │  "estimasi effort", "person-days", "sprint"           │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ content_creator             │ "Post LinkedIn", "konten media sosial", "hashtag",    │\n'
        '    │                             │  "draft artikel", "blog post", "caption"              │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ technical_writer            │ "Dokumen teknis", "BAB", "Daftar Isi", "PDF dibuat",  │\n'
        '    │                             │  "laporan teknis", "dokumentasi proyek"               │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ sysinfo_agent               │ "CPU:", "RAM:", "Disk:", "Memory:", "load average",   │\n'
        '    │                             │  "uptime", "resource server"                          │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ pdf_summarizer              │ "Ringkasan PDF", "isi dokumen", "halaman X-Y",        │\n'
        '    │                             │  "kesimpulan dokumen", "PDF berhasil diproses"        │\n'
        '    ├─────────────────────────────┼──────────────────────────────────────────────────────┤\n'
        '    │ doc_agent (doc_audit)       │ "Laporan Analisis Dokumen", "Daftar Isi", "Bab",      │\n'
        '    │                             │  "Ringkasan per Bab", "Tip: Balas pesan ini"          │\n'
        '    └─────────────────────────────┴──────────────────────────────────────────────────────┘\n'
        '\n'
        '    STEP 2 – Classify the current user message using the previous agent as anchor:\n'
        '\n'
        '    A) SPECIFIC FOLLOW-UP (direct reference to previous task output):\n'
        '       Applies when the user message explicitly refers to something from the previous response\n'
        '       (e.g. "klik tombol itu", "fix bug nomor 2", "elaborasi poin ketiga", "jelaskan bab 3",\n'
        '        "buka link pertama yang ditemukan", "kapan bisa deploy?").\n'
        '       → Classify as the SAME intent as the previous specialist agent (see table above).\n'
        '       → Set confidence ≥ 0.90.\n'
        '\n'
        '    B) GENERIC FOLLOW-UP (vague continuation / clarification request):\n'
        '       Applies when the user message is a vague continuation — NOT mentioning a completely\n'
        '       different topic — and uses phrases such as:\n'
        '         Indonesian: "jelaskan lebih", "jelaskan lebih lanjut", "lebih detail", "detail lagi",\n'
        ...
        '           "boleh tau kenapa", "hah?", "oh ya?", "benarkah?", "masa?", "serius?",\n'
        '           "gimana maksudnya", "coba jelaskan lagi", "kurang paham", "bisa lebih rinci",\n'
        '           "tolong perjelas", "apa hubungannya", "kasih contoh", "apa bedanya", "kenapa itu penting"\n'
        ...
        '           "can you expand on that", "I don\'t understand", "what does that mean",\n'
        '           "why does that matter", "could you rephrase", "is that right", "are you sure",\n'
        '           "can you be more specific", "in simpler terms", "what\'s the difference",\n'
        '           "what\'s the connection", "show me an example", "prove it"\n'
        '       — AND the previous [Asisten] response came from a specialist agent (contains research\n'
        '         findings, code analysis, bug reports, WBS/mandays output, system data, web automation\n'
        '         results, PDF summaries, document analysis, content drafts, technical docs, etc.) —\n'
        '       → NEVER classify as "general_inquiry" (which routes to the basic responder).\n'
        '       → Route using this priority order:\n'
        '           1. If a SAME specialist intent exists and is still contextually relevant → use it.\n'
        '              Example: previous was code_inspection → next is also code_inspection.\n'
        '           2. If the user is asking a factual question that REQUIRES fresh web data → use "research".\n'
        '           3. If the user needs an elaboration that relies on TOOLS (search, analysis, code) → "research".\n'
        '           4. Only use "general_inquiry" (responder) when the question is truly generic small-talk\n'
        '              with NO connection to the previous specialist topic whatsoever.\n'
        '\n'
        '    C) NEW TOPIC — override previous context when the user clearly switches topics:\n'
        '       Applies when the user mentions a URL, repo link, new PDF, a completely different\n'
        '       subject, or uses explicit task-start phrases like "sekarang", "next", "now do",\n'
        '       "buat baru", "mulai dari awal", "ganti topik", "lupa yang tadi".\n'
        '       → Classify normally based on the NEW message content only.\n'
        '\n'
        '    D) DOCUMENT AUDIT follow-up (special case):\n'
        '       When the most recent [Asisten] message contains document analysis signals\n'
        '       ("Laporan Analisis Dokumen", "Daftar Isi", "Ringkasan per Bab",\n'
        '        "Tip: Balas pesan ini untuk bertanya") AND the current message is a question\n'
        '       or request about document content → classify as "doc_audit".\n'
        '\n'
        '    E) WEB AUTOMATION follow-up (special case):\n'
        '       When the most recent [Asisten] message contains "📍 URL Aktif" OR describes\n'
        '       browser interactions, AND the current user message is a browsing command\n'
        '       (e.g. "berikan screenshot", "klik tombol X", "scroll ke bawah", "isi form",\n'
        '        "lanjutkan", "foto halaman", "tangkap layar") → classify as "web_automation"\n'
        '       with confidence ≥ 0.93.',
